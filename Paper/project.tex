...
In recent years, with the development of China, air pollution becomes a serious problem that government should deal with \shortcite{p1}. Under such condition, figuring out how air pollutants are distributed becomes an important task to the environmental management. In previous research, distributions of different air pollutants such as PM2.5, PM10, SO2 and NO2 over different locations and timescale has been studied in a partial manner \shortcite{p2,p3,p4,p5}.
...
There are many multivariate interpolation method for 2D interpolation, such as bilinear, bicubic methods \shortcite{wiki:1}. However, when applying these methods, there are always some empty parts in the map. It is because these methods are used to interpolate regular grid, but the position of stations does not form a regular grid. For this sort of scattered data, radial basis function(RBF) is used for interpolation in the paper \shortcite{wiki:2}. The function form is rather simple. For any point $\mathbf{x}$,
\begin{eqnarray}
